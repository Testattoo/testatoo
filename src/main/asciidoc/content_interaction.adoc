...
----
include::{sourcedir}/groovy/doc/SelectorTest.groovy[tags=componentSelection]
----

<1> Match the DOM element on the page with *id: 'button'*
<2> Match the DOM element on the *class: 'btn-secondary'*
<3> Match the DOM element on the page that is *the second button*
<4> Match the DOM element on the page that have *attribute "data-role" equals to "myRole"*
...
attribute, or an id. In all this case a css3 selector is more than enough.

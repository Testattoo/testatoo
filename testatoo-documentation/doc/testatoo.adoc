...
Testatoo work with Strongly typed components:: As you can see Testatoo manipulate typed components like TextField, Button, ListView.
...
Testatoo use deferred evaluation:: You can instantiate ListView event if the component is not available in the page.
                                   You can also test that the component is missing.

Testatoo DSL is very clean and functional:: TODO
Testatoo code never refer to a technical layer:: Used to enter text or control items on the screen.


[NOTE]
===============================
As a rule when you create a test with Testatoo we recommend to add some default import to facilitate the auto completion on your favorite IDE(A) (ninja)
